...
Write-Host "       LINK SEQUENCER v1.7              " -ForegroundColor $headerColor
Write-Host "========================================" -ForegroundColor $headerColor
Write-Host ""
...
function Generate-Links {
    ...
    # Extract the number and link parts (fixed extraction using Groups[1].Index for correct pre/post-number parts)
    ...
    # Generate the links with forced out= filename on the same line
    ...
    1..$num_link | ForEach-Object {
        ...
        [void]$links.Add("$url out=$filename")
    }
